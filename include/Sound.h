...
class Sound
{
	char filename[256]; // nom du fichier contenant le son a jouer.
	int error;			// core d'erreur de 'system', non utilise.
public:
...
#ifdef __APPLE__
		sprintf(cmd, "afplay -v %g %s &", volume * 5, filename); // 5 semble etre la valeur maximale.
#else
